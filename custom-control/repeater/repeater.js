...
wp.customize.controlConstructor['bkc-repeater'] = wp.customize.Control.extend({
    ...
    ready: function() {
        ...
        /**
         * Minimize /Expand repeater.
         */
        this.container.on('click', '.customize-control-repeater .repeater-row-header', function(e) {
        ...
        this.container.on('click', '.customize-control-repeater .repeater-add-reset', function(e) {
            ...
            control.container.find('.repeater-hook-label').text('');
         ...
         */
        this.container.on('change keyup paste', '.bkc-repeater-input-fields', function() {

            var self = jQuery(this);
            if ('action_hook' == self.data('field')) {
                var value = self.val();
                if ('' != value) {
                    self.closest('.repeater-row').find('.repeater-hook-label').text( ' - ' + value );
                } else {
                    self.closest('.repeater-row').find('.repeater-hook-label').text('');
                }
            }
    ...
    updateValue: function() {
        ...
        control.repeaterContainer.find('.repeater-row').each(function(i) {
            ...
            newValue[i] = {};
            jQuery(this).find('.bkc-repeater-input-fields').each(function() {
    ...
    addNewRepeater: function() {
        ...
        clone_row.find('.repeater-hook-label').text('');
